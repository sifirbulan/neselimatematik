...
const providerNames:Record<string,string>={gemini:"Gemini",groq:"Groq",mistral:"Mistral",openrouter:"OpenRouter",openai:"OpenAI","deterministic-math-engine":"Neşevren Matematik Motoru",consensus:"Çoklu AI karşılaştırması","deterministic-verification":"Neşevren doğrulama"};
...
export function SolutionResult({ result }: { result: SolveResponse }) {
  ...
  const statusText = answer.verificationStatus === "verified"
    ...
    : result.consensusStatus === "agreement"
      ...
      : "AI çözümü";
  const source=providerNames[result.finalAnswerSource]??result.finalAnswerSource??"Neşevren";
  ...
  return (
    <section className="solutionCard" aria-live="polite">
      ...
      <div className="solutionMeta"><span>{result.analysis.topic} · {result.analysis.subtopic}</span><span className="providerPill">✦ {source}</span><span>Güven: %{Math.round(answer.confidence * 100)}</span></div>
    </section>
  );
}

...
export async function orchestrateQuestion(input:StudentQuestion):Promise<OrchestratorResult>{
 ...
 const attempts:ProviderAttempt[]=[];const successful:ProviderAttempt[]=[];
 for(const providerId of registered){const attempt=await runProvider(providerId,input,initialAnalysis);attempts.push(attempt);if(attempt.answer)successful.push(attempt);if(successful.length>=policy.desiredProviders)break}
